...
function ReportRoomContent() {
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 selection:bg-white/20">
        <div className="bg-white/5 p-8 rounded-3xl border border-white/10 shadow-sm text-center max-w-md w-full">
          <div className="w-16 h-16 bg-white/10 text-white rounded-2xl flex items-center justify-center mx-auto mb-6">
            <AlertTriangle className="w-8 h-8" />
          </div>
          <h2 className="text-xl font-bold text-white mb-2">Error Loading Report</h2>
          <p className="text-slate-400 mb-6">{error}</p>
          <Link href="/dashboard" className="inline-flex items-center gap-2 bg-white text-black px-6 py-3 rounded-xl hover:bg-slate-200 transition-colors font-medium">
            Return to Dashboard
          </Link>
        </div>
      </div>
    ...
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6">
        <motion.div 
          ...
          className="w-24 h-24 rounded-full bg-gradient-to-br from-white to-neutral-400 blur-xl mb-8"
        />
        ...
        <div className="w-full max-w-md space-y-4">
          <div className="h-24 bg-white/5 rounded-2xl animate-pulse shadow-sm border border-white/5" />
          <div className="grid grid-cols-3 gap-4">
            <div className="h-32 bg-white/5 rounded-2xl animate-pulse shadow-sm border border-white/5" />
            <div className="h-32 bg-white/5 rounded-2xl animate-pulse shadow-sm border border-white/5" />
            <div className="h-32 bg-white/5 rounded-2xl animate-pulse shadow-sm border border-white/5" />
          </div>
          <div className="h-48 bg-white/5 rounded-2xl animate-pulse shadow-sm border border-white/5" />
        </div>
      </div>
    );
  }
      ...
      <nav className="border-b border-white/10 bg-black/80 backdrop-blur-xl sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link href="/settings" title="Account Settings" className="w-8 h-8 rounded-full bg-white border border-white/30 flex items-center justify-center overflow-hidden hover:ring-2 hover:ring-white/50 transition-all cursor-pointer ml-2 shadow-[0_0_10px_rgba(255,255,255,0.1)]">
      ...
      <div className="w-full max-w-6xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-12">
          ...
          <div className="text-right relative z-10">
            ...
            <div className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-white to-neutral-500">{report.overallScore ?? 0}<span className="text-2xl text-slate-500">/100</span></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 relative z-10">
          ...
          <div className="lg:col-span-2 flex flex-col justify-between gap-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="p-6 rounded-3xl bg-white/5 border border-white/10 shadow-lg backdrop-blur-xl flex-1 flex items-center gap-6">
               <div className="w-14 h-14 rounded-2xl bg-white/10 flex items-center justify-center text-white shadow-inner shrink-0"><Target className="w-7 h-7" /></div>
               <div>
                 <h3 className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Technical Depth</h3>
                 <p className="text-3xl font-extrabold mt-1 text-white">{report.technicalDepth ?? 0}<span className="text-lg text-slate-500 font-normal">/100</span></p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="p-6 rounded-3xl bg-white/5 border border-white/10 shadow-lg backdrop-blur-xl flex-1 flex items-center gap-6">
               <div className="w-14 h-14 rounded-2xl bg-white/10 flex items-center justify-center text-white shadow-inner shrink-0"><MessageSquare className="w-7 h-7" /></div>
               <div>
                 <h3 className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Communication</h3>
                 <p className="text-3xl font-extrabold mt-1 text-white">{report.communication ?? 0}<span className="text-lg text-slate-500 font-normal">/100</span></p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="p-6 rounded-3xl bg-white/5 border border-white/10 shadow-lg backdrop-blur-xl flex-1 flex items-center gap-6">
               <div className="w-14 h-14 rounded-2xl bg-white/10 flex items-center justify-center text-white shadow-inner shrink-0"><TrendingUp className="w-7 h-7" /></div>
               <div>
                 <h3 className="text-slate-400 text-xs font-semibold uppercase tracking-wider">Confidence</h3>
                 <p className="text-3xl font-extrabold mt-1 text-white">{report.confidence ?? 0}<span className="text-lg text-slate-500 font-normal">/100</span></p>
        ...
        {/* Premium Section */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className={`relative rounded-3xl overflow-hidden mt-12 bg-white/5 border shadow-lg backdrop-blur-xl z-10 ${(isPremiumUnlocked || isAdmin) ? 'border-white/30' : 'border-white/10'}`}>
           {(!isPremiumUnlocked && !isAdmin) && (
             <div className="absolute inset-0 z-20 backdrop-blur-xl bg-black/80 flex flex-col items-center justify-center p-8 text-center border border-white/5 rounded-3xl">
                <div className="w-16 h-16 bg-white/10 rounded-full flex items-center justify-center mb-6 shadow-[0_0_30px_rgba(255,255,255,0.1)]">
                ...
                <button onClick={handleUnlockPremium} disabled={eligibility.loading} className="bg-white hover:bg-slate-200 text-black px-8 py-3.5 rounded-full font-medium shadow-[0_0_20px_rgba(255,255,255,0.1)] transition-all disabled:opacity-50">
           ...
           <div className={`p-8 ${(!isPremiumUnlocked && !isAdmin) && 'blur-md opacity-50 pointer-events-none'}`}>
              ...
              <div className="space-y-8">
                <div>
                  <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">Strengths</h3>
                  <motion.ul 
                    initial="hidden" 
                    animate="visible" 
                  ...
                  >
                    {report.strengths?.map((str: string, i: number) => (
                      ...
                      >
                        <span className="text-white mt-1">•</span> <span className="leading-relaxed">{str}</span>
                      </motion.li>
                    ))}
                  </motion.ul>
                </div>
                ...
                <div>
                  <h3 className="text-lg font-bold text-slate-300 mb-4 flex items-center gap-2"><AlertTriangle className="w-5 h-5" /> Areas for Improvement</h3>
                  <motion.ul 
                    initial="hidden" 
                    animate="visible" 
                    variants={{ visible: { transition: { staggerChildren: 0.1 } } }} 
                  ...
                  >
                    {report.improvements?.map((imp: string, i: number) => (
                      <motion.li 
                        ...
                        key={i} 
                        className="bg-neutral-950 p-5 rounded-2xl border border-white/5"
                      >
                  ...
                  return (
                    <>
                      ...
                      <motion.div variants={{ hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0 } }} className="space-y-6">
                        <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">Behavioral & Non-Verbal Analysis</h3>
                        <div className="bg-white/5 p-5 rounded-2xl border border-white/10">
                          <p className="text-sm text-slate-300 leading-relaxed font-light">{report.behavioralAnalysis}</p>
                        </div>
                        ...
                        {total > 0 || tabSwitches > 0 ? (
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                            ...
                            <div className="bg-white/5 border border-white/5 p-6 rounded-2xl space-y-4 flex flex-col justify-between hover:border-white/20 transition-all">
                              <div className="space-y-2">
                                ...
                                <div className="flex items-baseline gap-2">
                                  <span className={`text-3xl font-black ${tabSwitches === 0 ? 'text-white' : 'text-slate-400'}`}>
                            ...
                            <div className="bg-white/5 border border-white/5 p-6 rounded-2xl space-y-4 flex flex-col justify-between hover:border-white/20 transition-all">
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${
                                    focusRate >= 75 ? 'bg-white/10 text-white border-white/20' :
                                    focusRate >= 45 ? 'bg-white/5 text-slate-300 border-white/10' :
                            ...
                            <div className="bg-white/5 border border-white/5 p-6 rounded-2xl space-y-4 flex flex-col justify-between hover:border-white/20 transition-all">
                              <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                  ...
                                  <span className={`px-2.5 py-1 text-[10px] font-bold rounded-full border ${
                                    smileRate >= 5 ? 'bg-white/10 text-white border-white/20' :
                                ...
                                <div className="flex items-baseline gap-2">
                                  <span className={`text-3xl font-black ${smileRate >= 5 ? 'text-white' : 'text-slate-300'}`}>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white/5 border border-white/5 p-6 rounded-2xl space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${
                              report.speechAnalytics.wpm < 90 ? 'bg-white/5 text-slate-300 border border-white/10' :
                              report.speechAnalytics.wpm <= 150 ? 'bg-white/10 text-white border border-white/20' :
                        ...
                        <div className="bg-white/5 border border-white/5 p-6 rounded-2xl space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${
                              report.speechAnalytics.totalFillers <= 5 ? 'bg-white/10 text-white border border-white/20' :
                              report.speechAnalytics.totalFillers <= 15 ? 'bg-white/5 text-slate-300 border border-white/10' :
                              'bg-white/[0.03] text-slate-400 border border-white/5'
                            }`}>
                            ...
                            <div className="grid grid-cols-3 gap-2">
                              {Object.entries(report.speechAnalytics.fillers || {}).map(([word, count]: [string, any]) => (
                                <div key={word} className="bg-black/20 p-2 rounded-lg border border-white/5 flex flex-col items-center">
                                  ...
                                  <span className={`text-sm font-bold mt-0.5 ${count > 3 ? 'text-white' : count > 0 ? 'text-slate-300' : 'text-slate-500'}`}>
                                    {count}x
                                  </span>
                                </div>
                              ))}
...
export default function ReportPage() {
  return (
    <Suspense fallback={
      <div className="min-h-screen bg-black text-slate-100 flex flex-col items-center justify-center p-6 font-sans">
        <motion.div 
          animate={{ scale: [1, 1.05, 1], opacity: [0.5, 1, 0.5] }} 
          transition={{ duration: 2, repeat: Infinity, ease: "easeInOut" }}
          className="w-24 h-24 rounded-full bg-gradient-to-br from-white to-neutral-400 blur-xl mb-8"
